...
class WebsiteCrawler:
    def __init__(self, seeds: Iterable[str], max_depth: int = 2) -> None:
        configured_seeds = [url.strip() for url in (seeds) if url and url.strip()]
    ...
    def _extract_content_text(cls, soup: BeautifulSoup) -> tuple[str | None, str | None]:
        ...
        if len(text) <= 100:  # MIN_CONTENT_CHARS
    ...
    def crawl(self) -> dict:
        ...
        return self.documents, self.skipped_pages, len(self.visited)